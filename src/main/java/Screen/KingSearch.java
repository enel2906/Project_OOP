...
public class KingSearch extends JFrame {
	...
	public KingSearch() {
		...
		Gson gson = new Gson();
		ArrayList<Kings> kingList = new ArrayList<>();
		for(File file : fileList){
			try{
				String content = new String(Files.readAllBytes(file.toPath()));
				Kings king = gson.fromJson(content, Kings.class);
				kingList.add(king);
			} catch (IOException e) {
		...
		for(Kings king : kingList){
			suggestionList.add(king.getTen());
		}
		...
		table.addMouseListener(new MouseAdapter() {
			...
			public void mouseClicked(MouseEvent e) {
				if(e.getClickCount() == 1){
					...
					for (Kings king : kingList) {
						if (king.getTen().equalsIgnoreCase(selectedText)) {
							dispose();
