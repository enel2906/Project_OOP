...
public class PeriodsSearch extends JFrame {
	...
	public PeriodsSearch() {
		...
		ArrayList<Period> peList = new ArrayList<>();
		...
		for(Period pe : peList){
			suggestionList.add(pe.getTrieuDai());
		}
		...
		table.addMouseListener(new MouseAdapter() {
			...
			public void mouseClicked(MouseEvent e) {
				if(e.getClickCount() == 1){
					...
					for (Period period : peList) {
						if (period.getTrieuDai().equalsIgnoreCase(selectedText)) {
							dispose();
